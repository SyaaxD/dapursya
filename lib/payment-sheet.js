...
function isMissingSheetError(error, sheetName) {
  ...
  return (
    status === 400 &&
    message.toLocaleLowerCase("id-ID").includes(
      sheetName.toLocaleLowerCase("id-ID")
    ) &&
    /unable to parse range|not found|does not exist|tidak ditemukan/i.test(
      message
    )
  );
}
...
export async function appendPaymentRows(sheets, spreadsheetId, rows) {
  ...
  try {
    ...
    if (!isMissingSheetError(error, PAYMENT_SHEET)) throw error;

    // Setup hanya dijalankan jika tab memang belum ada. Pada penggunaan
    // normal, submit cukup melakukan satu append tanpa metadata/header writes.
